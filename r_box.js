...
Box.prototype.CIRCLE = function(obj) {
 ...
 b.CIRCLE({
  ...
  'r':r,
  "center":center_value,
  "center_units":(pixels||values),
  "radius":radius,
  "radius_units":(pixels||values)
  'line_width':null,
  'stroke_style':null,
  'fill_style':null
 });
 ...
 */
 ...
 if (obj.radius_units == "pixels") {
   

   this.ctx.beginPath();
   this.ctx.arc(p.x, p.y, obj.radius, 0, 2*Math.PI);
   this.ctx.fill();
   this.ctx.stroke();
   return;
 }
...
/*
  ...
  textboxes work like this. when we define box, we also define TextBox, and when we add a text, 

  box contains all the textboxes. box.textboxes = []
  
  when we add a textbox, we are making a TextBox, and pushing it into the textboxes array
  
  when we update the position of a TextBox, we have to refer to it by its index
...
*/
  ...
  function TextBox(obj) {
    ...
    */
    ...
    this.x = obj.x;
    this.y = obj.y;
    // this.position = obj.value;
    this.alignment = obj.alignment;
    this.alignment_id = obj.alignment_id;
  ...
  Box.prototype.ADD_TEXTBOX = function(obj_) {
    ...
    */
    ...
    let t = new TextBox(obj_);
    console.log(t);
    this.textboxes.push(t);
    
    this.container.appendChild(t.el);
  ...
  Box.prototype.UPDATE_TEXTBOX = function(obj_) {
    
    /*
    
    obj_ = {"index":i,"str":str,"x":x,"y":y}
    
    */
    let t = this.textboxes[obj_.index];
    t.str = obj_.str;
    t.el.innerHTML = obj_.str;
    
    t.x = obj_.x;
    t.y = obj_.y;
    // console.log(t);
    
    let pixel = this.VALUE2PIXEL({'x':t.x,'y':t.y});
    
    // default
    if (t.alignment === 'top-left') {
      t.el.style.top = pixel.y + 'px';
      t.el.style.left = pixel.x + 'px';
      return;
    }
  
    if (t.alignment_id == 6) {
      let h = t.el.getBoundingClientRect().height;
      let w = t.el.getBoundingClientRect().width;
      t.el.style.top = (pixel.y - 0.5*h) + 'px';
      t.el.style.left = (pixel.x + 0.25*w) + 'px';
      return;
    }
  
    if (t.alignment === 'center-right') {
      let h = t.el.getBoundingClientRect().height;
      let w = t.el.getBoundingClientRect().width;
      t.el.style.top = (pixel.y - 0.5*h) + 'px';
      t.el.style.left = (pixel.x - w) + 'px';
      return;
    }
    
    if (t.alignment === 'center-left') {
      let h = t.el.getBoundingClientRect().height;
      let w = t.el.getBoundingClientRect().width;
      t.el.style.top = (pixel.y - 0.5*h) + 'px';
      t.el.style.left = (pixel.x) + 'px';
    ...
    if (t.alignment === 'top-center') {
      let h = t.el.getBoundingClientRect().height;
      let w = t.el.getBoundingClientRect().width;
      t.el.style.top = pixel.y + 'px';
      t.el.style.left = (pixel.x - 0.5*w) + 'px';
    }
    
    if (t.alignment === 'top-right') {
      let h = t.el.getBoundingClientRect().height;
      let w = t.el.getBoundingClientRect().width;
      t.el.style.top = pixel.y + 'px';
      t.el.style.left = (pixel.x - w) + 'px';
  ...
  }; // closing update textbox position
